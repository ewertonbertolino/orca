...
describe('parseGitHubOwnerRepo', () => {
  ...
  // Why: literal host check without SSH config expansion — documents #10284 pre-fix.
...
describe('SSH Host alias identity (#10284)', () => {
  ...
  it('preserves SCP Host alias case for OpenSSH Host matching', () => {
    // OpenSSH Host patterns are case-sensitive; lowercasing would miss Host GitHub-Work.
    expect(gitHubSshConfigHostAlias('git@GitHub-Work:team/orca.git')).toBe('GitHub-Work')
  })
  ...
  it('returns owner/repo when resolved HostName is ssh.github.com (SSH-over-HTTPS)', () => {
    // Issue repro: Host github-work → HostName ssh.github.com Port 443
    expect(
