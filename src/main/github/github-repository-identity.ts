...
async function resolveOwnerRepoForRemote(
  ...
  try {
    ...
    // Why: Host aliases (git@github-work:…) are not github.com literally;
    // expand via ssh -G so PR merge/mutations still get owner/repo (#10284).
    const classification = await classifyGitHubOwnerRepoFromRemoteUrl(remoteUrl)
    ...
    if (classification.kind === 'indeterminate') {
      // Why: ssh -G timeout/ENOENT is not a stable "not GitHub" signal — do not
      // pin a multi-minute negative cache off git config signature alone.
      return null
    }
    // kind === 'not-github': stable until remotes/config change.
    ownerRepoCache.set(cacheKey, {
      value: null,
      expiresAt: now + getOwnerRepoCacheTtl(null, configSignature),
      ...(configSignature ? { configSignature } : {})
